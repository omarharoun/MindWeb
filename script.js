...
class EnhancedKnowledgeApp {
    ...
    setupEventListeners() {
        ...
        // Auto-arrange toggle
        document.getElementById('auto-arrange-toggle').addEventListener('click', () => {
            this.toggleAutoArrange();
        });
    }
    ...
    switchTab(tabName) {
        ...
        // Update header stats if switching to home tab
        if (tabName === 'home') {
    ...
    createNodeElement(node) {
        ...
        nodeDiv.style.setProperty('--node-color', category.color);
    ...
    updateHeaderStats() {
        ...
        if (headerStats) {
            headerStats.textContent = `${this.nodes.length} nodes • Level ${this.stats.level}`;
        }
    }
}
